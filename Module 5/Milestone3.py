...
class CWMachine(StateMachine):
    ...
    ##
    ## stopCurrent - flag used to stop the current message transmission
    ## and toggle to the next message
    ##
    stopCurrent = False
    ...
    ## state machine. This method stops the current message transmission,
    ## toggles the message, and allows transmission of the new message
    ##
    ...
    def transmit(self):
        ...
        while not self.endTransmission:
            ...
            for word in wordList:
                ...
                ## Check if we need to stop current transmission
                if self.stopCurrent:
                    self.stopCurrent = False
                    break
                ...
                for char in word:
                    ...
                    for x in morse:
                        ...
                        if x == '.':
                            self.doDot()
                            self.doDot()  # Return to off state
                        elif x == '-':
                            self.doDash()
                            self.doDash()  # Return to off state
                        else:
                            if(DEBUG):
                                print(f"Unknown Morse Symbol: {x}")

                        # If we are still sending process a dotDashPause event

                        ## Determine if there is another symbol in the message. If so
                        ## transition the state machine to the appropriate 

                        # Increment morseCounter
                        if morseCounter < lenMorse:
                            self.doDDP()
                            self.doDDP()  # Return to off state
                            morseCounter += 1 

                    # If we are still sending process a letterPause event

                    ## Determine if there is another letter in the message. If so
                    ## transition the state machine to the appropriate 
                    ## state, and increase the wordCounter variable by 1.

                    if wordCounter < lenWord:
                        self.doLP()
                        self.doLP()  # Return to off state
                        wordCounter += 1

                # If we are still sending process a wordPause event

                ## Determine if there is another word in the message. If so
                ## transition the state machine to the appropriate 
                ## state, and increase the wordsCounter variable by 1.

                if wordsCounter < lenWords:
                    self.doWP()
                    self.doWP()  # Return to off state
                    wordsCounter += 1
...
## Assign the appropriate function to the greenButton variable 
